...
InstagramStream.prototype.createHeader = function() {
	var es = this;
	if (es.connected) {
		this.stream.button("home", "Home", "ws_icon19 home-grey-20x20");
		this.stream.button("popular", "Popular", "ws_icon19 star-grey-20x20");
		this.stream.button("mine", "My Photos", "ws_icon19 my_photos-25x20");
		this.stream.button("like", "My Likes", "ws_icon19 heart-grey-20x20");
		this.stream.button("search", "Search Instagram", "ws_icon19 search");
	}
	this.stream.button("settings", "Settings", "ws_icon19 settings");
	this.stream.button("more", "More", "ws_icon19 dropdown");
	this.stream.on("home", function() {
		es.feedType = "home";
		es.loadMore(true);
	...
	this.stream.on("popular", function() {
		es.feedType = "popular";
		es.loadMore(true);
		es.updateStreamSetting();
		jQuery(".hs_dropdown>div").hide();
	...
	this.stream.on("search", function(view, target, data, e) {
		jQuery(".hs_dropdown>div").hide();
		if (target.active()) {
	...
	this.stream.on("settings", function(view, target, data, e) {
		jQuery(".hs_dropdown>div").hide();
		if (target.active()) {
			if (es.connected) {
	...
	this.stream.on("more", function(view, target, data, e) {
		jQuery(".hs_dropdown>div").hide();
		if (target.active()) {
...
InstagramStream.prototype.init = function() {
	...
	jQuery("#widget").html("");
	this.stream = weisuite.streamApp();
	var is=this;
	this.stream.on("next",function(){
		is.loadMore(false);
	});
	return this;
}
